...
import { STORIES_DATA } from '../../component/stories/constants';
import Breadcrumbs from '@/component/global/breadcrumbs';

export default function BlogPost() {
    const pathname = usePathname();
    const id = parseInt(pathname?.split("/")?.pop()); // id को number में
    const postIndex = STORIES_DATA.findIndex((val) => val.id == id);
    const post = STORIES_DATA[postIndex];

    // Previous & Next Blog
    const prevPost = STORIES_DATA[postIndex - 1];
    const nextPost = STORIES_DATA[postIndex + 1];
