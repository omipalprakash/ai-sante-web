...
const Stories = () => {
  ...
  return (
    <div className="flex flex-col gap-4 md:gap-[30px] lg:gap-[50px]" id="stories">
      <h2
        className="text-left sm:text-center font-['Erstoria',_sans-serif] text-[#453030]"
        data-aos="fade-down"
      >
      ...
      <div className="hidden lg:block relative pb-4">
        <Slider
          {...{
            dots: false,
            arrows: false,
            infinite: STORIES_DATA.length > 4,
            speed: 500,
            slidesToShow: 4,
            slidesToScroll: 1,
          }}
          ...
          {STORIES_DATA.map((item, index) => (
            <div key={index} className="px-3 h-full">
              <div className="flex flex-col bg-[#F8F6F9] rounded-2xl shadow-sm p-4 h-full min-h-[480px]">
                ...
                {/* Title */}
                <h3 className="uppercase text-black text-[20px] leading-6 font-salmond font-normal">
                  {item.title}
                </h3>

                {/* Description */}
                <p className="text-sm text-gray-600 mt-2">
                  {item.content.split(" ").slice(0, 13).join(" ") + "..."}
                </p>
                ...
                <div className="flex flex-wrap gap-2 my-3">
                  {item.tags.map((tag, idx) => (
                    <div
                      key={idx}
                      className={`flex items-center gap-2 text-sm text-black ${styles.tag}`}
                    >
                      {tag}
